...
# get absolute path + adding path to uploads directory
UPLOAD_FOLDER = os.path.abspath(os.getcwd())+'/static/uploads'
...
# making this model because if foods are empty then we might have something in database that this was something food we have in db
class deletedfooditems(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    username = db.Column(db.String(80), nullable=False)
    itemName = db.Column(db.String(120), nullable=False)
    expDay = db.Column(db.Integer, nullable=False)
...
class orders(db.Model):  # username of owner's food that's unique.
    id = db.Column(db.Integer, primary_key=True)
    # username of food owner
    username = db.Column(db.String(80), nullable=False)
    # it will be the buyer's fullname
    fullname = db.Column(db.String(120), nullable=False)
    # it will be the buyer's email
    email = db.Column(db.String(100), nullable=False)
    # it will be the buyer's location
    location = db.Column(db.String(200), nullable=False)
    # it will be the buyer's contact
    contact = db.Column(db.String(20), nullable=False)
    # it will be the food item id which buyer has get
    fooditemid = db.Column(db.Integer, nullable=False)
    # it will be the food item quantity
    quantityoffood = db.Column(db.String(1000), nullable=False)
...
def display(filename):
    ...
    for obj in list_of_data:
        try:
            ...
            new_data.append({"id": item.id, "username": item.username, "itemName": item.itemName,
                             "expDay": item.expDay, "expMonth": item.expMonth, "expYear": item.expYear, "qx": obj['qx']})
        except:
    ...
    if group == "Store":
        ...
        if isFound != None:
            if password == isFound.password:
                ...
                if len(items) == 0:
                    items = deletedfooditems.query.filter_by(
                        username=username).all()
                order = orders.query.filter_by(username=username).all()
                obj = {}
                for item in order:
                    try:
                        value = fooditem.query.filter_by(
                            id=item.fooditemid).first().itemName
                    except:
                        value = deletedfooditems.query.filter_by(
                            id=item.fooditemid).first().itemName
...
def listitem(username):
    ...
    items = fooditem.query.filter_by(username=username).all()
    order = orders.query.filter_by(username=username).all()
    # if food is not in footitem it's mean everyfood has taken by someone.
    if len(items) == 0:
...
def genrateQR(data):
    ...
    for obj in items:
        ...
        if int(obj['qx']) < item.qx:
            ...
            # item.qx = item.qx - int(obj['qx'])
            # db.session.commit()
            deletefooditem = deletedfooditems(
                item.id, item.username, item.itemName, item.expDay, item.expMonth, item.expYear, 0)
